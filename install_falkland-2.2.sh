...
if [ "$1" != '-q' ] && [ "$1" != "-i" ] || [ $# = 0 ]; then
	echo "install_falkland.sh - Script for Installing Gentoo-Falkland"
	echo "USAGE: sh install_falkland.sh -i|-q dhcp,IPs [-pd] [-om]"
	echo "-h : help"
...
if [ $OWNMIRROR = 0 ]; then
	INSTSRV="distfiles.nitso.org"
	FLURL="http://$INSTSRV/linux/falkland/x86_64-server/stages/$STG3"
...
if [ "`netstat -tan|grep -c ':22 '`" = "0" ]; then
        $GRUEN
        echo "SSH is down. Activating now"
        $NRML
...
if [ "$MODE" = "I" ]; then
	...
	echo "Which mount point (e.g. /mnt/falkland) shell be used for Root-System (/)"
...
$NRML
# wget -q $PURL
git clone http://git.nitso.org/falkland-portage.git $MNTRT/usr/portage
