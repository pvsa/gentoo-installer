...
echo "Compiling Kernel"
 ...
 LINUX="`ls $MNTRT/usr/src`"
 chroot $MNTRT /bin/bash -c "ln -s /usr/src/$LINUX /usr/src/linux"
 wget http://kernel.alioth.debian.org/config/3.0.0-5/config_amd64_none_amd64 -O $MNTRT/usr/src/linux/.config
 chroot $MNTRT /bin/bash -c "cd /usr/src/linux; make all; make modules_install; make install"
