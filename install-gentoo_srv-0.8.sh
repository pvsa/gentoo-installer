...
## 0.7
# change stage...tgz to xz
## 0.8
# Grub2, ntpd, screen installieren. Domain beim Aufrug als Parameter
### DEBUG
#set -x
#
...
if [ "$MODE" = "" ] || [ "$NOOPT" = 1 ] || [ "$HELP" = 1 ] ; then
    ...
    echo " -n=hostname : Name of the host aka. hostname"
	echo " -d=domain : Name of the corresponding domain (FQDN)"
	echo " -i : interactive"
	...
	echo "	    w
stage3-latest.tar.bz2 and portage-latest.tar.bz2 (assuming both lying on the same place) "
...
if [ "$MODE" = "I" ]; then
	...
	read MNTRT
	echo "Which is boot device for grub"
	read BOOTDEV
fi
...
$NRML
STG3="`ls stage3*.tar.xz`"
tar --numeric-owner -xpf $STG3
mv $STG3 $MNTRT/root/
...
if [ "$MODE"  = "I" ]; then
	...
	if [ "$NET" = "dhcp" ]; then
		echo 'config_eth0="dhcp"' >> $MNTRT/etc/conf.d/net
	    	...
	    	#Namesever
	    	## Quad9
	    	echo "nameserver 9.9.9.9" > $MNTRT/etc/resolv.conf
	    	## Cloudflare
	    	echo "nameserver 1.1.1.1" >> $MNTRT/etc/resolv.conf
            if [ -n $DNAME ]; then 
				echo "$IP $HNAME.$DNAME $HNAME" >> $MNTRT/etc/hosts
			else
				echo "$IP $HNAME" >> $MNTRT/etc/hosts
			fi 
	fi
    echo "hostname=\"$HNAME\" " > $MNTRT/etc/conf.d/hostname
	chroot $MNTRT /bin/bash -c "hostname $HNAME"
fi
chroot $MNTRT /bin/bash -c "ln -s /etc/init.d/net.lo /etc/init.d/net.$NETDEV"
chroot $MNTRT /bin/bash -c "env-update;source /etc/profile;rc-update add net.$NETDEV boot default"

if [ -n $DNAME ]; then 
	echo "dns_domain_lo=\"$DNAME\" " >> $MNTRT/etc/conf.d/net
	chroot $MNTRT /bin/bash -c "env-update;source /etc/profile;rc-update add net.lo boot default"
fi 

DNS="`cat /etc/resolv.conf |grep nameserver |cut -d " "  -f 2`"
echo "Setting DNS-Server to $DNS (like this live system)"
cp /etc/resolv.conf $MNTRT/etc/

# locale en_US.UTF-8
$GRUEN && echo "Setting locale (C + EN)"
$NRML
echo "C.UTF-8 UTF-8" >> $MNTRT/etc/locale.gen
echo "en_US.UTF-8 UTF-8" >> $MNTRT/etc/locale.gen
chroot $MNTRT /bin/bash -c "env-update;source /etc/profile;locale-gen"
...
$NRML
    ...
    rm /tmp/pck.txt
   $GELB && echo "Attention: Precompiled Kernel was not installed with gentoo-sources."
	echo "Please check installed kernel and adjust to your needs (source+profile)"
   $NRML
...
$NRML
chroot $MNTRT /bin/bash -c "env-update;source /etc/profile;emerge -qD sys-boot/grub"

# set kernel-version for new kernel 
...
# fstab
echo "#Generated by Gentoo installer" > $MNTRT/etc/fstab
...
#fi
chroot $MNTRT  /bin/bash -c "env-update;source /etc/profile;grub-install $BOOTDEV"
chroot $MNTRT  /bin/bash -c "env-update;source /etc/profile;grub-mkconfig -o /boot/grub/grub.cfg"
...
$GRUEN && echo "emerge eix, gentoolkit, screen, cronie"
$NRML
chroot $MNTRT /bin/bash -c "env-update;source /etc/profile;emerge -qD --quiet-build eix gentoolkit app-misc/screen cronie"

$GRUEN && echo "emerge nullmailer and activate at startup"
$NRML
chroot $MNTRT /bin/bash -c "env-update;source /etc/profile;emerge -qD --quiet-build nullmailer"
chroot $MNTRT /bin/bash -c "env-update;source /etc/profile;emerge --config nullmailer"
$GELD && echo "Attention: nullmailer is not finally configured to operate"
$NRML
chroot $MNTRT /bin/bash -c "rc-update add nullmailer"

# ntpd install 
$GRUEN && echo "emerge NTPd and activate at startup"
$NRML
chroot $MNTRT /bin/bash -c "env-update;source /etc/profile;emerge -q net-misc/ntp"
chroot $MNTRT /bin/bash -c "rc-update add ntpd"
chroot $MNTRT /bin/bash -c "rc-update add ntp-client"


$GRUEN && echo "Checking for necessary Net and Disk modules"
$NRML
# needed net drivers
lsmod|grep net|grep -v '#'|cut -d ' ' -f 1 > /tmp/netmods.lst
while read MOD
do
 echo "$MOD:"
...
# needed hdd drivers
lsmod|grep ata|grep -v '#'|cut -d ' ' -f 1 > /tmp/hddmods.lst
while read MOD
do
 echo "$MOD:"
 find $MNTRT/lib/modules/ |grep "$MOD"
...
if [ "$MODE" = "I" ]; then
	...
	echo "pilarkto" >> .pw-file
	chroot $MNTRT /bin/passwd -q < .pw-file
	rm -f .pw-file
...
$NRML
echo "Visit Projekt-Wiki: https://wiki.pilarkto.org/Intern:IT:HowTo:Gentoo_Install.html"
echo "Have fun"
$NRML
